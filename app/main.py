...
app = FastAPI(title="DMAP-AI API", version="0.9.1")
...
def _enforce_demo(mode: Optional[str], aoi: Optional[str], datasource: Optional[str]):
    """Demo-mode guard (historical + point + single datasource)."""
    if not DEMO_LOCK:
        return
...
def spi(
    ...
    return {
        ...
        "note": "Simple z-score SPI from pre-aggregated sums. "
                "Gamma-fit SPI also available at /demo/spi_gamma_historical_auto.",
    }
...
# Supported datasource codes (frontend: <select id="obsSrc">)
...
def _fetch_power_precip(lat: float, lon: float, start: dt.date, end: dt.date) -> Dict[str, float]:
    ...
    if not isinstance(param, dict):
        raise HTTPException(
            status_code=502,
            detail={
                "message": "Unexpected NASA POWER payload",
                "sample_keys": list(j.keys())[:5],
            },
        )
...
def _resolve_datasource(datasource: Optional[str]) -> str:
    """Normalize datasource code and enforce supported list."""
    ...
    if ds == USER_DATASOURCE:
        raise HTTPException(
            status_code=400,
            detail={
                "message": "User-upload ('user') datasource is not implemented in this API. "
                           "The current backend only supports gridded sources "
                           "(ERA5, GridMET, PRISM, GPM) via server-side fetch.",
            },
        )
    raise HTTPException(
        status_code=400,
...
def _fetch_precip_by_source(
    ...
    """
    Central hook for datasource routing.

    For now, ALL supported gridded datasources are served via NASA POWER as a
    placeholder (so ERA5 / GridMET / PRISM / GPM will produce the same series).
    Later, you can branch this function to call native APIs or preprocessed
    datasets for each source.
    """
    ds = _resolve_datasource(datasource)
    # TODO: branch by ds ('era5', 'gridmet', 'prism', 'gpm') to real backends.
...
def _aligned_baseline_window_sums(
    ...
    for yr in range(by0, by1 + 1):
        try:
            last_day = (dt.date(yr, end.month, 1) + dt.timedelta(days=31)).replace(
                day=1
            ) - dt.timedelta(days=1)
...
def spi_historical_auto(
    ...
    datasource: str = Query("era5", description="era5|gridmet|prism|gpm|user"),
):
    if DEMO_LOCK and datasource.lower() != ALLOWED_DATASOURCE:
        raise HTTPException(
            status_code=403,
            detail={"message": "Demo mode: ERA5 only for auto SPI."},
        )
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={"message": "Invalid end_date. Use YYYY-MM-DD."},
        )
    ...
    base_all = _fetch_precip_by_source(
        lat, lon, dt.date(by0, 1, 1), dt.date(by1, 12, 31), datasource
    )
    ...
    if len(baseline_sums) < 2:
        raise HTTPException(
            status_code=502,
            detail={"message": "Insufficient baseline length for SPI."},
        )
    try:
    ...
    if sd == 0:
        raise HTTPException(
            status_code=502,
            detail={"message": "Insufficient baseline variance for SPI z-score."},
        )
    ...
    return {
        ...
        "spi": round(spi_z, 3),
        "note": "Legacy demo using z-score SPI. "
                "Currently, all gridded datasources are backed by NASA POWER daily precipitation. "
                "Prefer /demo/spi_gamma_historical_auto for Gamma SPI.",
    }
...
def spi_gamma_historical_auto(
    ...
    if DEMO_LOCK and datasource.lower() != ALLOWED_DATASOURCE:
        raise HTTPException(
            status_code=403,
            detail={"message": "Demo mode: ERA5 only for auto SPI (Gamma)."},
        )
    ...
    if len(baseline_sums) < 5:
        raise HTTPException(
            status_code=502,
            detail={"message": "Insufficient baseline length for Gamma fit."},
        )
    ...
    if len(pos) < 3:
        ...
        if sd == 0:
            raise HTTPException(
                status_code=502,
                detail={
                    "message": "Baseline variance is zero; cannot compute SPI."
                },
            )
        mu = mean(baseline_sums)
        spi_z = (sum_obs - mu) / sd
        return {
            ...
            "note": "Fallback to z-score due to insufficient positive samples for Gamma fit. "
                    "All gridded datasources are currently backed by NASA POWER precipitation.",
        }
    ...
    return {
        ...
        "note": "SPI via 2-parameter Gamma fit on baseline window sums (zeros handled). "
                "All gridded datasources (ERA5, GridMET, PRISM, GPM) are currently served via NASA POWER.",
    }
...
def spi_gamma_series(
    ...
    yearly_method: Literal["total", "window"] = Query(
        "total",
        description="'total'=Jan–Dec totals; 'window'=rolling window",
    ),
    ...
    anchor: Optional[str] = Query(
        None,
        description="Optional 'MM-DD' string; overrides anchor_mm/dd if given",
    ),
):
    # Demo lock: ERA5 only
    if DEMO_LOCK and datasource.lower() != ALLOWED_DATASOURCE:
        raise HTTPException(
            status_code=403,
            detail={"message": "Demo mode: ERA5 only for SPI series."},
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={"message": "Invalid start_date/end_date. Use YYYY-MM-DD."},
        )
    if end < start:
        raise HTTPException(
            status_code=400,
            detail={"message": "end_date must be >= start_date."},
        )
    ...
    # fetch obs & baseline once (via datasource router)
    obs_all = _fetch_precip_by_source(
        lat, lon, dt.date(start.year, 1, 1), dt.date(end.year, 12, 31), datasource
    )
    by0, by1 = _parse_baseline(baseline)
    base_all = _fetch_precip_by_source(
        lat, lon, dt.date(by0, 1, 1), dt.date(by1, 12, 31), datasource
    ...
    if step == "month":
        ...
        month_params = {}
        for m in range(1, 13):
            p0, shape, scale, pos, zeros = _fit_gamma_from_samples(
                baseline_month_samples[m]
            )
            month_params[m] = {
                "p_zero": p0,
                "shape": shape,
        ...
        for y, m in _month_iter(start, end):
            ...
            if (y == start.year and m == start.month and start != s) or (
                y == end.year and m == end.month and end != e
            ):
                continue
            tot = _sum_window(s, e, obs_all)
            params = month_params[m]
            spi_val, Gx, H = _gamma_spi_for_value(
                tot, params["p_zero"], params["shape"], params["scale"]
            )
            if spi_val is None:
            ...
            series.append(
                {
                    ...
                    else round(params["shape"], 6),
                    "gamma_scale": None
                    if params["scale"] is None
                    else round(params["scale"], 6),
        ...
        return {
            ...
            "note": "SPI per calendar month using baseline-fitted Gamma per month. "
                    "All gridded datasources are currently backed by NASA POWER daily precipitation.",
        }
    ...
    if yearly_method == "total":
        ...
        return {
            ...
            "note": "SPI per calendar year (Jan–Dec totals). 'window_days' is ignored here. "
                    "All gridded datasources currently share a NASA POWER backend.",
        }
    ...
    return {
        ...
        "note": "SPI per year using rolling N-day window ending on anchor date. "
                "All gridded datasources are currently served via NASA POWER.",
    }
...
def spi_wavelet(req: WaveletRequest):
    ...
    if not req.spi or len(req.spi) < 8:
        raise HTTPException(
            status_code=400,
            detail={"message": "Need at least 8 SPI values for wavelet analysis."},
        )
    ...
    # Transpose to (time, period) for frontend
    ...
    return {
        ...
        "scalogram": scalogram_data,  # <-- [Time][Period]
        "coherence": coherence,
...
def spi_gamma_both(
    ...
    monthly = spi_gamma_series(
        ...
        step="month",
        yearly_method="total",
    )  # yearly_method ignored for month
    yearly = spi_gamma_series(
        lat=lat,
        lon=lon,
        start_date=start_date,
        end_date=end_date,
