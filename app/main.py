...
def _resolve_datasource(datasource: Optional[str]) -> str:
    ...
    - 'era5', 'gridmet', 'prism', 'gpm' are treated as gridded sources.
    - 'user' (station/CSV upload) is **not implemented** on the server yet and
      will raise a clear HTTP 400 instead of silently falling back.
    """
        ...
        raise HTTPException(
            ...
            detail={
                "message": "User-upload ('user') datasource is not implemented in this API. "
                           "The backend currently only supports gridded sources via server-side fetch."
            },
        )
    raise HTTPException(
        status_code=400,
        detail={
            "message": f"Unknown datasource '{datasource}'. "
                       "Supported codes: era5, gridmet, prism, gpm, user."
        },
    )
...
def _fetch_precip_by_source(
    ...
    """Central hook for all gridded datasources.

    For now, **all** gridded datasources (ERA5, GridMET, PRISM, GPM) are
    served via NASA POWER as a placeholder, so they will produce *numerically
    identical* series until you wire each branch to a real backend
    (e.g., preprocessed NetCDF on disk).

    This at least ensures that:
      * 'user' does NOT silently behave like ERA5.
      * invalid datasource strings generate a clear HTTP 400.
    """
    ds = _resolve_datasource(datasource)
    # TODO: later branch by ds ('era5', 'gridmet', 'prism', 'gpm') and
    #       call the appropriate data backend instead of POWER.
...
def _annual_sum(year: int, daily_map: Dict[str,float]) -> float:
    s = dt.date(year,1,1); e = dt.date(year,12,31)
...
def _zscore(x: float, samples: List[float]) -> Optional[float]:
    if len(samples) < 2:
        return None
    try:
        mu = mean(samples); sd = stdev(samples)
    except Exception:
        return None
    if sd == 0:
        return None
    ...
    if step == "month":
        ...
        return {
            ...
            "note": "SPI per calendar month using baseline-fitted Gamma per month. "
                    "All gridded datasources are currently backed by NASA POWER.",
        }
...
def spi_gamma_both(
    ...
    monthly = spi_gamma_series(
        lat=lat, lon=lon,
        start_date=start_date, end_date=end_date,
        window_days=30, baseline=baseline,
        datasource=datasource, step="month", yearly_method="total"
    )
    yearly = spi_gamma_series(
        lat=lat, lon=lon,
        start_date=start_date, end_date=end_date,
        window_days=30, baseline=baseline,
        datasource=datasource, step="year", yearly_method="total"
    )
